...
import { getAuth, signInWithEmailAndPassword, signOut, onAuthStateChanged, GoogleAuthProvider, signInWithPopup } from "https://www.gstatic.com/firebasejs/9.20.0/firebase-auth.js";
...
function createNoticeElement(notice) {
    ...
    div.innerHTML = `
        ...
        ${adminControls}
    `;
    const noticeContent = div.querySelector('.notice-content');
    if (notice.content.length > 200) {
        const preview = notice.content.substring(0, 200) + '...';
        const full = notice.content;
        noticeContent.innerHTML = `
            <span class="preview-text">${preview}</span>
            <a href="#" class="read-more">Read More</a>
            <span class="full-text" style="display: none;">${full}</span>
        `;

        const readMoreLink = noticeContent.querySelector('.read-more');
        readMoreLink.addEventListener('click', function(e) {
            e.preventDefault();
            const previewSpan = noticeContent.querySelector('.preview-text');
            const fullSpan = noticeContent.querySelector('.full-text');
            const isCollapsed = previewSpan.style.display !== 'none';
